...
/*   Updated: 2024/10/20 15:54:03 by anqabbal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_the_floor(t_exec *exec, unsigned int y, unsigned int x, int var)
{
	...
	{
		xx = 0;
		while (xx < var && yy < (int)exec->info.map_hei)
		{
			if (yy != 0 && xx > 0 && ((int)y + yy) < (int)exec->info.map_hei)
				mlx_put_pixel(exec->wind_image, x + xx, y + yy, 0x04ed93ff);
			else
			{
				if (((int)y + yy) < (int)exec->info.map_hei)
