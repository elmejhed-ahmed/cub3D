...
/*   Updated: 2024/10/20 16:08:42 by anqabbal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_the_player(t_exec *exec, int var, int new_y)
{
	...
	while ((int)j < AOV)
	{
		...
		while (i < get_persent(PIXELS, 3) && nx > 0 && ny > 0 && nx < exec->info.map_wid && ny < exec->info.map_hei)
		{
...
void    draw_mini_map(t_exec *exec)
{
	...
	new_y = exec->info.map_hei - (((exec->info.map_hei / PIXELS)) * (PIXELS / get_persent(PIXELS, 10)));
